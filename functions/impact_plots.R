...
SCC_plot <- function(scenplot=scenlist, regions = witch_regions, normalization_region = "World"){
 ...
 #GDP
  ...
  SCC_bar_chart <- ggplot(subset(scc_value_marginals, file %in% scenplot_nopulse)) + geom_bar(aes(file, SCC, fill=file), position = "dodge", stat="identity") + ylab("SCC [$/tCO2eq]") + xlab("") + geom_text(aes(file, SCC*0.9, label=paste0(round(SCC,1), "$"))) + guides(fill=FALSE) + ylim(0, max(scc_value_marginals$SCC))
  ...
  if(any(str_detect(scenplot, "_emission_pulse"))){
    ...
    scc_value_pulse <- scc_pulse %>% group_by(pathdir, file) %>% filter(t>= t0 & t <= tmax) %>% summarize(SCC=sum(scc_contribution_normalized*1e12/pulse_size)) %>% as.data.frame()
    scc_value_pulse$pathdir <- "Emission_pulse"
    print(scc_value_pulse)
    SCC_bar_chart_pulse <- ggplot(scc_value_pulse) + geom_bar(aes(file, SCC, fill=file), position = "dodge", stat="identity") + ylab("SCC [$/tCO2eq] (PULSE)") + xlab("")  + geom_text(aes(file, SCC*0.9, label=paste0(round(SCC,1), "$"))) + guides(fill=FALSE) + ylim(0, max(c(scc_value_marginals$SCC, scc_value_pulse$SCC)))
    SCC_bar_chart <- ggplot(subset(scc_value_marginals, file %in% scenplot_nopulse)) + geom_bar(aes(file, SCC, fill=file), position = "dodge", stat="identity") + ylab("SCC [$/tCO2eq]") + xlab("") + geom_text(aes(file, SCC*0.9, label=paste0(round(SCC,1), "$"))) + guides(fill=FALSE) + ylim(0, max(c(scc_value_marginals$SCC, scc_value_pulse$SCC)))
  }
