...
diagnostics_plots <- function(scenplot=scenlist){
  ...
  runs <- iterations %>% group_by(pathdir, file, run) %>% summarize(total_time=sum(time), n_iter=length(siter), Optimal=sum(optimal, na.rm = T), n_iter_feas=sum(feasible, na.rm = T)) %>% mutate(Infeasible=n_iter-n_iter_feas, Nonoptimal=n_iter-Optimal-Infeasible)
