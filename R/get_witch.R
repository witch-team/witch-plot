...
get_witch <- function(variable_name, 
                      variable_name_save = variable_name, 
                      scenplot = scenlist, 
  ...
  if(!exists(variable_name) | (variable_name %in% c("t", "n", "p", "I")) | force_reload){
    ...
    for (current_pathdir in fullpathdir){
      for (file in filelist){
        if(file.exists(file.path(current_pathdir, paste0(file,".gdx")))){
          ...
          {
            tempdata <- mygdx[variable_name, field = field]
    ...
    if(exists("allfilesdata")){
      ...
      if(stringr::str_detect(variable_name, "eq")) {
      ...
      if(stringr::str_detect(variable_name, "MAGICC|HECTOR")) {allfilesdata <- suppressWarnings(allfilesdata[,-c("magicc_n", "hector_n")])}
      ...
      if(combine_old_new_j & (variable_name %in% varlist_combine_old_new_j)){
        ...
        if(length(j_set)>0){
          ...
          if(variable_name=="Q_EN") allfilesdata <- allfilesdata %>% filter(!stringr::str_detect(get(j_set), paste(c("_old", "_new", "_late"), collapse = "|")))   
      ...
      if(("t" %in% names(allfilesdata)) & (!any(stringr::str_detect(allfilesdata$t, "_")))) allfilesdata$t <- as.numeric(allfilesdata$t)
