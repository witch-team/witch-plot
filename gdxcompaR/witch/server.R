...
shinyServer(function(input, output, session) {
    ...
    output$gdxompaRplot <- renderPlot({
      ...
      if (is.null(additional_set_selected2)) {
        additional_set_selected2 <- set_elements2[1]
      }
      if ((additional_set_id2!="na" & additional_set_selected2[1]=="na") |
          !(additional_set_selected2[1] %in% set_elements2)) {
        additional_set_selected2 <- set_elements2[1]
      }
      ...
      if (regions[1]=="World" | regions[1]=="EU" | 
          length(regions)==1) { #if only World is displayed or only one region, show files with colors
        ...
        p <- p + geom_line(stat="identity", linewidth=1.5) + 
          xlab("year") + 
          ylab(unit_conv$unit) + 
