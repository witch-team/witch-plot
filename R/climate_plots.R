...
climate_plot <- function(scenplot=scenlist, regions = "World"){
  get_witch("Q_EMI", check_calibration = T)
  ...
  #get also BAU values
  get_witch("BAU_Q_EMI", check_calibration = T)
  ...
  get_witch("TRF", check_calibration = T)
  ...
  get_witch("MAGICCTRF", check_calibration = T)
  ...
  get_witch("HECTORTRF", check_calibration = T)
  ...
  get_witch("HECTORTEMP", check_calibration = T)
  ...
  if(exists("MAGICCTRF")) trf_plot_magicc <- plot_witch(climate_data, varname = "trf_magicc6", scenplot = scenplot, regions = "World", ylab = "Total Radiative Forcing [W/m2] (MAGICC6)", conv_factor=1, nagg="mean") else trf_plot_magicc = trf_plot
  if(exists("HECTORTRF")) trf_plot_hector <- plot_witch(climate_data, varname = "trf_hector", scenplot = scenplot, regions = "World", ylab = "Total Radiative Forcing [W/m2] (HECTOR)", conv_factor=1, nagg="mean") else trf_plot_hector = trf_plot
  ...
  if(exists("MAGICCTEMP")) temp_plot_magicc <- plot_witch(climate_data, varname = "temp_magicc6", scenplot = scenplot, regions = "World", ylab = "Temp. increase [degC] (MAGICC6)", conv_factor=1, nagg="mean") else temp_plot_magicc = temp_plot
  if(exists("HECTORTEMP")) temp_plot_hector <- plot_witch(climate_data, varname = "temp_hector", scenplot = scenplot, regions = "World", ylab = "Temp. increase [degC] (HECTOR)", conv_factor=1, nagg="mean") else temp_plot_hector = temp_plot
