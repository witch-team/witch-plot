...
shinyServer(function(input, output, session) {
    ...
    output$gdxompaRplot <- renderPlot({
      ...
      if(regions[1]=="World" | length(regions)==1){#if only World is displayed or only one region, show files with colors
        ...
        p <- p + geom_line(data=subset(allfilesdata, n %in% regions & str_detect(file, "historical")),aes(year,value,colour=n, group=interaction(file,n)), linetype = "solid", stat="identity", size=1.0)
    ...
    output$gdxompaRplotly <- renderPlotly({
      ...
      if(regions[1]=="World" | length(regions)==1){#if only World is displayed or only one region, show files with colors
        ...
        p_dyn <- p_dyn + geom_line(data=subset(allfilesdata, n %in% regions & str_detect(file, "historical")),aes(ttoyear(t),value,colour=n, group=interaction(file,n)), linetype = "solid", stat="identity", size=1.0)
